...
public class ThirdCam : MonoBehaviour {
	...
	public GQController controller;
	...
	void Awake ()
	{
		...
		if (_target)
		{
			controller = _target.GetComponent<GQController>();
		}
	...
	void Apply (Transform dummyTarget, Vector3 dummyCenter)
	{
		...
		var currentRotation = Quaternion.Euler (0f, currentAngle, 0f);
		...
		Vector3 tempPosition = cameraTransform.position;
	...
	void SetUpRotation (Vector3 centerPos,Vector3 headPos)
	{
		...
		var yRotation = Quaternion.LookRotation(new Vector3(offsetToCenter.x, 0f, offsetToCenter.z));
		
		var relativeOffset = Vector3.forward * distance + Vector3.down * height;
		cameraTransform.rotation = yRotation * Quaternion.LookRotation(relativeOffset);
		
		// Calculate the projected center position and top position in world space
		var centerRay = cameraTransform.camera.ViewportPointToRay(new Vector3(.5f, 0.5f, 1f));
		var topRay = cameraTransform.camera.ViewportPointToRay(new Vector3(.5f, clampHeadPositionScreenSpace, 1f));
